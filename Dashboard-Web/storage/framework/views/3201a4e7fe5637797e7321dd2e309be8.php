...
<?php $__env->startSection('title', 'Cek Tunggakan'); ?>
<?php $__env->startSection('page-title', 'Cek Tunggakan Santri'); ?>
...
<div class="container-fluid" style="padding: 20px;">
    ...
    <div class="tunggakan-filter-card no-print">
        <form action="<?php echo e(route('bendahara.cek-tunggakan')); ?>" method="GET">
            <div class="row" style="display: flex; gap: 16px; flex-wrap: wrap; align-items: flex-end;">
                ...
                <div style="display: flex; gap: 8px;">
                    ...
                    <?php if(request()->anyFilled(['kelas_id', 'asrama_id', 'gender'])): ?>
                        <a href="<?php echo e(route('bendahara.cek-tunggakan')); ?>" style="background: rgba(255,255,255,0.2); color: white; padding: 12px 16px; border-radius: 10px; text-decoration: none; font-weight: 600;">
    ...
    <div class="row mb-4" style="display: flex; gap: 16px; flex-wrap: wrap;">
        <div style="flex: 1; min-width: 200px;">
            <div class="stat-card">
                <div class="stat-value"><?php echo e($totalSantriMenunggak); ?></div>
                <div class="stat-label">Santri Menunggak</div>
            </div>
        </div>
        <div style="flex: 1; min-width: 200px;">
            <div class="stat-card warning">
                <div class="stat-value"><?php echo e($grandTotalBulan); ?></div>
                <div class="stat-label">Total Bulan Tunggakan</div>
            </div>
        </div>
        <div style="flex: 1; min-width: 200px;">
            <div class="stat-card danger">
                <div class="stat-value" style="color: #dc2626;">Rp <?php echo e(number_format($grandTotalRupiah, 0, ',', '.')); ?></div>
    ...
    <div class="tunggakan-table">
        <div style="padding: 20px; border-bottom: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;">
            <div>
                ...
                <small style="color: #64748b;">Biaya Syahriah: Rp <?php echo e(number_format($biayaBulanan, 0, ',', '.')); ?>/bulan</small>
            </div>
            <a href="<?php echo e(route('bendahara.cek-tunggakan.export', request()->query())); ?>" target="_blank" class="no-print" style="background: #0f172a; color: white; padding: 10px 20px; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; text-decoration: none; display: inline-block;">
        ...
        <table>
            ...
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $santriWithArrearsPaginated; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr>
                        <td style="text-align: center;"><?php echo e($santriWithArrearsPaginated->firstItem() + $index); ?></td>
                        <td style="font-weight: 500;"><?php echo e($item['santri']->nis); ?></td>
                        <td style="font-weight: 600; color: #1e293b;"><?php echo e($item['santri']->nama_santri); ?></td>
                        <td><?php echo e($item['santri']->kelas->nama_kelas ?? '-'); ?></td>
                        <td style="text-align: center;">
                            <span class="badge-tunggakan"><?php echo e($item['unpaid_months']); ?> bulan</span>
                        </td>
                        ...
                        <td style="text-align: center;" class="no-print">
                            <div style="display: flex; gap: 8px; justify-content: center;">
                                <?php if($item['santri']->no_hp_ortu_wali): ?>
                                <?php
                                ...
                                ?>
                                <a href="<?php echo e($waUrl); ?>" target="_blank" style="background: linear-gradient(135deg, #25D366 0%, #128C7E 100%); color: white; border: none; padding: 8px 12px; border-radius: 8px; font-size: 12px; font-weight: 600; display: flex; align-items: center; gap: 4px; text-decoration: none;" title="Kirim Tagihan via WhatsApp">
                                ...
                                <?php endif; ?>
                                <form action="<?php echo e(route('bendahara.cek-tunggakan.process')); ?>" method="POST" style="display: inline;" data-turbo="false" target="_blank">
                                    <?php echo csrf_field(); ?>
                                    <input type="hidden" name="santri_id" value="<?php echo e($item['santri']->id); ?>">
                                    <input type="hidden" name="biaya_bulanan" value="<?php echo e($biayaBulanan); ?>">
        ...
        <!-- Pagination -->
        <?php if($santriWithArrearsPaginated->hasPages()): ?>
        <div style="padding: 16px 20px; border-top: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;">
            <span style="color: #64748b; font-size: 14px;">
                Menampilkan <?php echo e($santriWithArrearsPaginated->firstItem()); ?> - <?php echo e($santriWithArrearsPaginated->lastItem()); ?> dari <?php echo e($santriWithArrearsPaginated->total()); ?> santri
            </span>
            <div style="display: flex; gap: 8px;">
                <?php if($santriWithArrearsPaginated->onFirstPage()): ?>
                    <span style="padding: 8px 14px; background: #f1f5f9; color: #94a3b8; border-radius: 8px; font-size: 14px;">← Sebelumnya</span>
                <?php else: ?>
                    <a href="<?php echo e($santriWithArrearsPaginated->previousPageUrl()); ?>" style="padding: 8px 14px; background: #667eea; color: white; border-radius: 8px; text-decoration: none; font-size: 14px; font-weight: 600;">← Sebelumnya</a>
                <?php endif; ?>
                
                <?php if($santriWithArrearsPaginated->hasMorePages()): ?>
                    <a href="<?php echo e($santriWithArrearsPaginated->nextPageUrl()); ?>" style="padding: 8px 14px; background: #667eea; color: white; border-radius: 8px; text-decoration: none; font-size: 14px; font-weight: 600;">Berikutnya →</a>
                <?php else: ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\v\.gemini\antigravity\scratch\dashboard-riyadlul-huda\resources\views\bendahara\cek-tunggakan\index.blade.php ENDPATH**/ ?>