...
<?php $__env->startSection('title', 'Gaji Pegawai'); ?>
<?php $__env->startSection('page-title', 'Gaji Pegawai'); ?>
    ...
    <div style="display: grid; grid-template-columns: 1fr; gap: 32px;">
        <div style="background: white; border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.03); border: 1px solid #f1f5f9; overflow: hidden;">
            ...
            <div style="padding: 24px;">
                <form method="POST" action="<?php echo e(route('bendahara.gaji.store')); ?>">
                    <?php echo csrf_field(); ?>
                    <div style="display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                        <div>
                            ...
                            <select name="pegawai_id" style="width: 100%; padding: 12px; border-radius: 10px; border: 1px solid #e2e8f0; font-size: 1rem;" required>
                                ...
                                <?php $__currentLoopData = $pegawaiList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($p->id); ?>"><?php echo e($p->nama_pegawai); ?> - <?php echo e($p->jabatan); ?></option>
                        ...
                        <div>
                            ...
                            <input type="number" name="tahun" style="width: 100%; padding: 12px; border-radius: 10px; border: 1px solid #e2e8f0; font-size: 1rem;" value="<?php echo e(date('Y')); ?>" required>
                        </div>
                    </div>
                    <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 20px; margin-bottom: 32px;">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_bayar" style="width: 100%; padding: 12px; border-radius: 10px; border: 1px solid #e2e8f0; font-size: 1rem;" value="<?php echo e(date('Y-m-d')); ?>">
                        </div>
                    </div>
                ...
                <table style="width: 100%; border-collapse: collapse;">
                    ...
                    <tbody style="font-size: 0.9375rem;">
                        <?php $__empty_1 = true; $__currentLoopData = $gaji; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $g): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr id="row-<?php echo e($g->id); ?>" style="border-bottom: 1px solid #f1f5f9; transition: background 0.2s;">
                                <td style="padding: 16px 24px;">
                                    <div style="font-weight: 700; color: #1e2937;"><?php echo e($g->pegawai->nama_pegawai ?? '-'); ?></div>
                                </td>
                                <td style="padding: 16px 24px; color: #475569;">
                                    <?php echo e($g->pegawai->jabatan ?? '-'); ?>

                                </td>
                                <td style="padding: 16px 24px; color: #64748b; font-weight: 500;">
                                    <?php echo e(date('F', mktime(0, 0, 0, $g->bulan, 1))); ?> <?php echo e($g->tahun); ?>

                                </td>
                                <td style="padding: 16px 24px; text-align: right; color: #1e2937; font-weight: 700;">
                                    Rp <?php echo e(number_format($g->nominal, 0, ',', '.')); ?>

                                </td>
                                <td style="padding: 16px 24px; text-align: center;">
                                    <span style="display: inline-block; padding: 6px 12px; border-radius: 9999px; font-size: 0.75rem; font-weight: 700; <?php echo e($g->is_dibayar ? 'background: #ecfdf5; color: #059669;' : 'background: #fff1f2; color: #e11d48;'); ?>">
                                        <?php echo e($g->is_dibayar ? 'Dibayar' : 'Menunggu'); ?>

                                    </span>
                                </td>
                                <td style="padding: 16px 24px; color: #64748b;">
                                    <?php echo e($g->tanggal_bayar ? $g->tanggal_bayar->format('d/m/Y') : '-'); ?>

                                </td>
                                <td style="padding: 16px 24px; text-align: center;">
                                    <div style="display: flex; justify-content: center; gap: 8px;">
                                        <button onclick="toggleEdit(<?php echo e($g->id); ?>)" style="background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 6px; color: #475569; cursor: pointer; transition: all 0.2s;">
                                            <i data-feather="edit-2" style="width: 16px; height: 16px;"></i>
                                        </button>
                                        <form method="POST" action="<?php echo e(route('bendahara.gaji.destroy', $g->id)); ?>" style="display: inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?')">
                                            <?php echo csrf_field(); ?>
                            ...
                            <!-- Edit Row -->
                            <tr id="edit-<?php echo e($g->id); ?>" style="display: none; background: #f8fafc;">
                                <td colspan="7" style="padding: 24px; border-bottom: 1px solid #e2e8f0;">
                                    <form method="POST" action="<?php echo e(route('bendahara.gaji.update', $g->id)); ?>">
                                        <?php echo csrf_field(); ?>
                                        ...
                                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 20px;">
                                            <div>
                                                ...
                                                <select name="is_dibayar" style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1; font-size: 0.875rem;" required>
                                                    <option value="1" <?php echo e($g->is_dibayar ? 'selected' : ''); ?>>Dibayar</option>
                                                    <option value="0" <?php echo e(!$g->is_dibayar ? 'selected' : ''); ?>>Menunggu</option>
                                                </select>
                                            </div>
                                            <div>
                                                ...
                                                <input type="date" name="tanggal_bayar" style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1; font-size: 0.875rem;" value="<?php echo e($g->tanggal_bayar ? $g->tanggal_bayar->format('Y-m-d') : ''); ?>">
                                            </div>
                                            <div style="grid-column: span 2;">
                                                ...
                                                <input type="text" name="keterangan" style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1; font-size: 0.875rem;" value="<?php echo e($g->keterangan); ?>">
                                            </div>
                                        </div>
                                        <div style="display: flex; justify-content: flex-end; gap: 12px;">
                                            <button type="button" onclick="toggleEdit(<?php echo e($g->id); ?>)" style="background: white; border: 1px solid #e2e8f0; color: #475569; padding: 10px 20px; border-radius: 8px; font-weight: 600; cursor: pointer;">Batal</button>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\v\.gemini\antigravity\scratch\dashboard-riyadlul-huda\resources\views\bendahara\gaji\index.blade.php ENDPATH**/ ?>