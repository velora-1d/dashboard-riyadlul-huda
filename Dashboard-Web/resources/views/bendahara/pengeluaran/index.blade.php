    ...
    <div style="display: grid; grid-template-columns: 1fr; gap: 32px;">
        ...
        <div style="background: white; border-radius: 20px; box-shadow: 0 10px 30px rgba(0,0,0,0.03); border: 1px solid #f1f5f9; overflow: hidden;">
            ...
            <div style="padding: 24px;">
                <form method="POST" action="{{ route('bendahara.pengeluaran.store') }}">
                    @csrf
                        ...
                        @forelse($pengeluaran as $p)
                            <tr id="row-{{ $p->id }}" style="border-bottom: 1px solid #f1f5f9; transition: background 0.2s;">
                                ...
                                <td style="padding: 16px 24px; text-align: center;">
                                    ...
                                    <div style="display: flex; justify-content: center; gap: 8px;">
                                        ...
                                        <form method="POST" action="{{ route('bendahara.pengeluaran.destroy', $p->id) }}" style="display: inline;" onsubmit="return confirmDelete(event, 'Data pengeluaran ini akan dihapus.')">
                                            @csrf
                                            @method('DELETE')
                            ...
                            <tr id="edit-{{ $p->id }}" style="display: none; background: #f8fafc;">
                                <td colspan="6" style="padding: 24px; border-bottom: 1px solid #e2e8f0;">
                                    <form method="POST" action="{{ route('bendahara.pengeluaran.update', $p->id) }}">
                                        @csrf
                                        @method('PUT')
                                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 16px; margin-bottom: 20px;">
                                            <div>
                                                ...
                                                <input type="text" name="jenis_pengeluaran" style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1; font-size: 0.875rem;" value="{{ $p->jenis_pengeluaran }}" required>
                                            </div>
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <select name="kategori" style="width: 100%; padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1; font-size: 0.875rem;" required>
                                                    <option value="operasional" {{ $p->kategori == 'operasional' ? 'selected' : '' }}>Operasional</option>
                                                    <option value="gaji" {{ $p->kategori == 'gaji' ? 'selected' : '' }}>Gaji / Honor</option>
                                                    <option value="pemeliharaan" {{ $p->kategori == 'pemeliharaan' ? 'selected' : '' }}>Pemeliharaan & Aset</option>
                                                    <option value="pendidikan" {{ $p->kategori == 'pendidikan' ? 'selected' : '' }}>Kegiatan Pendidikan</option>
